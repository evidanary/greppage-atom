...
export default class GrepPageView {
    ...
    this.element.querySelectorAll('#search-input')[0].onkeypress = function(e) {
      ...
      if (keyCode == '13'){
        ...
        GrepPage.searchGrepPage(e.target.value)
          .then(res => {
            _this.updateViewData(e.target.value, res.data.docs);
          })
  ...
  createItemElement(doc) {
    ...
    description.textContent = doc.description + ' - ' + doc.topic_name;
  ...
  updateViewData(query, items) {
    this.setQuery(query);
    
    if (!items.length) {
      const span = document.createElement('span');
      span.textContent = `No results found found - ${query}`
