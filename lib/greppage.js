...
export default {
  config: {
    guestAccessToken: {
      type: 'string',
      default: 'Bearer eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiIsImFsZyI6IkhTMjU2IiwidHlwIjoiSldUIn0.eyJpZCI6MjAwMDAwMDAwMCwiZW1haWwiOiJndWVzdEBndWVzdC5jb20iLCJuYW1lIjoiZ3Vlc3QiLCJleHAiOjE1MTExMzY4MzB9.gWohR7LLtROgjSl5SxbEaGRBveZQEv7Uj2rzmgYrbys',
    }
  },
  grepPageView: null,
  subscriptions: null,
  ...
  searchGrepPage(query) {
    return axios.get('https://www.greppage.com/api/search/items', {
      headers: {'Authorization': atom.config.get('greppage.guestAccessToken')},
      params: {
        'q.op': 'AND',
        wt: 'json',
        start: 0,
  ...
  showSidePanelWithSearchResponse(query, docs) {
    this.grepPageView.updateViewData(query, docs);
    this.sidePanel.show();
